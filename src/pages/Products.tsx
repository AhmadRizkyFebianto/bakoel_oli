...
const CATEGORIES = ['Motor Matic', 'Motor Bebek', 'Motor Sport', 'Pelumas Khusus'];
...
export default function Products({ addToCart }: ProductsProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <PageBanner
        title={<>Produk Perawatan di bengkel <span className="text-brand-yellow">Bakul Oli</span></>}
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-grow space-y-8">
            ...
            <h2 className="text-2xl font-bold">
              Pilihan Produk di bengkel <span className="text-brand-yellow">Bakul Oli</span>
            </h2>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <label key={item} className="flex items-center gap-3 text-sm cursor-pointer group">
            ...
            />
            <span className="group-hover:text-brand-blue transition-colors">{item}</span>
          </label>
        ))}
      </div>
    </div>
        ...
        {[1, 2, 3, '...', 20].map((p, i) => (
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
              p === 1 ? 'bg-brand-dark text-white' : 'bg-white text-gray-500 hover:bg-gray-100'
            }`}
          >
            {p}
          </button>
