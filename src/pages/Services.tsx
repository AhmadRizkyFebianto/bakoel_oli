...
export default function Services() {
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <PageBanner
        title={<>Layanan bengkel <span className="text-brand-yellow">Bakul Oli</span></>}
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2">
              ...
              <p className="text-gray-500 text-lg mb-8">
                Percayakan perawatan mesin kendaraan Anda pada Bengkel Bakul Oli. Layanan profesional
                dengan teknisi berpengalaman dan oli berkualitas untuk performa optimal.
              </p>
              <div className="grid grid-cols-2 gap-6">
                {TRUST_ITEMS.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className="bg-brand-yellow/20 p-2 rounded-lg text-brand-dark">{item.icon}</div>
...
function ServiceCard({ image, alt, title, description, buttonLabel, buttonClass }: ServiceCardProps) {
  return (
    <motion.div
      whileHover={{ y: -10 }}
    ...
    >
      ...
      <p className="text-gray-500 mb-10 text-sm leading-relaxed">{description}</p>
      <button className={`w-full py-4 rounded-2xl font-bold text-lg transition-all shadow-md active:scale-95 ${buttonClass}`}>
        {buttonLabel}
      </button>
    </motion.div>
  );
