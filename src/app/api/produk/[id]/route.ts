...
export async function PUT(
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (formData.has("nama_product")) nama_product = formData.get("nama_product") as string;
      if (formData.has("jenis_oli")) jenis_oli = formData.get("jenis_oli") as string;
      if (formData.has("peruntukan")) peruntukan = formData.get("peruntukan") as string;
      if (formData.has("cc_motor")) cc_motor = formData.get("cc_motor") as string;
      if (formData.has("kekentalan_oli")) kekentalan_oli = formData.get("kekentalan_oli") as string;
      if (formData.has("harga")) harga = Number(formData.get("harga"));
      if (formData.has("stok")) stok = Number(formData.get("stok"));
      if (formData.has("deskripsi")) deskripsi = formData.get("deskripsi") as string;
      ...
      if (body.kekentalan_oli !== undefined) kekentalan_oli = body.kekentalan_oli;
    ...
    if (kekentalan_oli !== undefined) dataToUpdate.kekentalan_oli = kekentalan_oli;
    if (harga !== undefined && !isNaN(harga)) dataToUpdate.harga = Math.round(harga);
    if (stok !== undefined && !isNaN(stok)) dataToUpdate.stok = Math.round(stok);
    if (deskripsi !== undefined) dataToUpdate.deskripsi = deskripsi;

    // Proses upload ke ImgBB jika ada file/base64 gambar baru yang diunggah
    if (imageInput) {
      try {
        ...
        return NextResponse.json(
          { message: "Gagal mengupload gambar ke ImgBB" },
          { status: 500 }
        );
      }
    }
